...
def generate_correlation_plot(config: ExperimentConfig, step: int = None):
    ...
    Returns:
        Dictionary with correlation statistics, or None if data is missing
    """
    if step is None:
    ...
    if len(logits_by_lr) < 3:
        print(
            f"Error: Need logits for all 3 learning rates at step {step}. Skipping plot.")
        print(f"  Available LRs: {list(logits_by_lr.keys())}")
        return None
...
def load_metrics_from_results_files(config: ExperimentConfig):
    ...
    plaintext_dir = results_dir / "plaintext" / \
        f"{config.DATASET}_{config.MODEL}"

    if not plaintext_dir.exists():
        print(f"Warning: Results directory not found: {plaintext_dir}")
        return {}

    metrics_by_lr = {lr: {'batch_sharpness': [], 'lambda_max': []}
                     for lr in config.LEARNING_RATES}
            ...
            with open(results_file, 'r') as f:
                for line in f:
                    ...
                    try:
                        ...
                        full_loss = float(
                            parts[3]) if parts[3] != 'nan' else np.nan
                        lmax = float(parts[4]) if parts[4] != 'nan' else np.nan
                        step_sharpness = float(
                            parts[5]) if parts[5] != 'nan' else np.nan
                        batch_sharpness = float(
                            parts[6]) if parts[6] != 'nan' else np.nan
                        gni = float(parts[7]) if parts[7] != 'nan' else np.nan
                        accuracy = float(
                            parts[8]) if parts[8] != 'nan' else np.nan
            ...
            if data:
                ...
                if 'batch_sharpness' in df.columns:
                    bs_df = df[['step', 'batch_sharpness']].dropna(
                        subset=['batch_sharpness'])
                    if not bs_df.empty:
                        metrics_by_lr[matched_lr]['batch_sharpness'].append(
                            bs_df.set_index('step')['batch_sharpness'])

                if 'lambda_max' in df.columns:
                    lmax_df = df[['step', 'lambda_max']].dropna(
                        subset=['lambda_max'])
                    if not lmax_df.empty:
                        metrics_by_lr[matched_lr]['lambda_max'].append(
                            lmax_df.set_index('step')['lambda_max'])

                print(
                    f"Loaded metrics from {run_dir.name} (LR={matched_lr}): {len(data)} data points, {len([d for d in data if not np.isnan(d['batch_sharpness'])])} batch_sharpness, {len([d for d in data if not np.isnan(d['lambda_max'])])} lambda_max")
...
def generate_sharpness_plots(config: ExperimentConfig):
    ...
    if not has_data:
        ...
        print(
            "Expected format: RESULTS/plaintext/{dataset}_{model}/timestamp_lr{lr}_b{batch}/results.txt")
        return
    ...
    has_bs_data = any(metrics_by_lr.get(lr, {}).get(
        'batch_sharpness', []) for lr in config.LEARNING_RATES)
    has_lmax_data = any(metrics_by_lr.get(lr, {}).get(
        'lambda_max', []) for lr in config.LEARNING_RATES)
    ...
    if has_bs_data:
        ...
        ax1.set_title(
            'Batch Sharpness vs Learning Rate (No Data)', fontsize=14)
